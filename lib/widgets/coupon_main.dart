...
class CouponMain extends StatefulWidget {
  final String id;

  CouponMain({this.id});

  @override
  _CouponMainState createState() => _CouponMainState();
}

class _CouponMainState extends State<CouponMain> {

  HomeApiAssistant homeApi = new HomeApiAssistant();

  bool _isCouponsLoading;

  List<Coupon> _coupons , _rCoupons = [];
  ...
  /// Coupons System
  ///-----------------------------------------------------------------
  Future _getCouponById(String id) async
  {

    var ssResponse = await http
        .post('https://yalaphone.com/appdash/rest_api/coupons/get_coupon_by_id.php' , body: {
      'coupon_id' : widget.id,
    });
  ...
  //                           fit: BoxFit.cover,
  //                           image: NetworkImage("https://yalaphone.com/appdash/"+_rCoupons[i].logo.toString()),
  //                         )
  //                     )),
  //
  //                 Container (
  ...
  _initializeCouponsSection(String id) async
  {
    await _getCouponById(id).then((value){
      setState(() {
  ...
  void initState() {
    ...
    print(widget.id);
    _initializeCouponsSection(widget.id);
      ...
      Builder(
        builder: (BuildContext context) {
          return SingleChildScrollView(
            child: Column(
              children: <Widget>[
                for(int i = 0; i < _rCoupons.length; i++) couponWidget(
                    i, context),
              ],
            ),
          );
