...
class _HomeState extends State<Home> {
  ...
  Future<void> showBigPictureNotification(
      String imgUrl, String title, String body) async {
  ...
  Future<NotificationDetails> _getImage(
  ...
  _foregroundNotificationRouter() {
    switch (notificationRouteType) {
        ...
        if (couponNotificationId != '0') {
          Navigator.of(context)
            ...
            new MaterialPageRoute(
                builder: (BuildContext context) => new CouponMain(
                      id: couponNotificationId,
                    )),
          )
              .whenComplete(() {
            setState(() {
      ...
      case 'local_store':
        if (storeNotificationId != null) {
          homeApi.getStoreById(storeNotificationId).then((store) {
            Navigator.of(context)
                .push(
              new MaterialPageRoute(
                  builder: (BuildContext context) => new StoryCoupon(
                        country: store,
                      )),
            )
                .whenComplete(() {
  ...
  void initializeNotificationsConfigs() {
    ...
    _firebaseMessaging.configure(
      onMessage: (Map<String, dynamic> message) async {
        api
            .sendNotificationDeliveryAck(message['data']['notification_id'])
            .then((value) {
      ...
      onLaunch: (Map<String, dynamic> message) async {
        if (!await checkIfDuplicatedNotification(
            message['data']['notification_id'])) {
          setLastNotificationId(message['data']['notification_id'])
              .whenComplete(() {
      ...
      if (value) {
        ...
        showDialog(
            ...
            builder: (_) => AssetGiffyDialog(
                  ...
                  buttonOkText: Text(
                      getTranslated(context, 'home_alert_login_ok_btn'),
                      style: TextStyle(
                  ...
                  image:
                      Image.asset('assets/images/alert.png', fit: BoxFit.cover),
                  title: Text(
  ...
  _loadMoreButtonChild() {
    if (loadModeChildIndicator == 0) {
      return Text(
        getTranslated(context, 'home_coupons_load_more_btn'),
  ...
  Future _initializeStoresSection() async {
    final prefs = await SharedPreferences.getInstance();
  ...
  Future _getCoupons(String current) async {
    var ssResponse = await http.post(
        'https://yalaphone.com/appdash/rest_api/coupons/coupons_lazy_load_all.php',
  ...
  Future _getCouponsByCountry(String countryCode, String current) async {
    ...
    var ssResponse = await http.post(
        'https://yalaphone.com/appdash/rest_api/coupons/coupons_lazy_load_by_country.php',
        body: {
          'country': countryCode,
          'current_id': current,
        });
  ...
  Future _initializeCouponsSection(String currentCoupon) async {
    final prefs = await SharedPreferences.getInstance();
    final key = 'country_code';
    final value = prefs.get(key);
      ...
      await _getCouponsByCountry(value, currentCoupon).then((value) {
        setState(() {
          ...
          for (var fav in _rCoupons) {
            setState(() {
              _favouritesLikeIcons.add(Icon(
                Icons.favorite,
                color: Colors.red,
              ));
              _favouritesDislikeIcons.add(Icon(
  ...
  String cPropertyByLocale(context, Coupon coupon, String type) {
    ...
    if (currentLocale.languageCode == 'ar') {
      switch (type) {
        case 'name':
          return coupon.arName;
          break;
        case 'des':
          return coupon.arDescription;
          break;
      }
    } else {
      switch (type) {
        case 'name':
          return coupon.enName;
          break;
        case 'des':
          return coupon.enDescription;
          break;
      }
    }
  }
                    ...
                    child: new Column(
                      children: <Widget>[
                        Padding(
                          ...
                          child: Text(
                            cPropertyByLocale(context, _rCoupons[i], 'name'),
                            style: TextStyle(
                                fontSize: 20,
                                fontFamily: "CustomFont",
                                fontWeight: FontWeight.bold),
                        ...
                        children: [
                          InkWell(
                            onTap: () {
                              ...
                              if (homeApi.checkIfInCodes(
                                      _rCoupons[i].id, _rCodes) ==
                                  null)
                                _copyCode(_rCoupons[i].id, _rCoupons[i].code);
                            },
                            child: Container(
                                ...
                                children: [
                                    ...
                                    children: <Widget>[
                                      ...
                                      Text(
                                        getTranslated(context,
                                                'home_coupon_code_used_prefix') +
                                            _copyTimes[i].toString() +
                                            getTranslated(context,
                                                'home_coupon_code_used_suffix'),
                                        style: TextStyle(
                                          fontSize: 16,
                                          color: Colors.black,
                                          fontFamily: "CustomFont",
                                  ...
                                  Row(
                                    ...
                                    children: <Widget>[
                                      ...
                                      Text(
                                        getTranslated(context,
                                                'home_coupon_code_add_date') +
                                            _rCoupons[i].createdAt.toString(),
                                        style: TextStyle(
                                          fontSize: 16,
                                          color: Colors.black,
                                          fontFamily: "CustomFont",
                                  ...
                                  InkWell(
                                    onTap: () {
                                      ...
                                      if (homeApi.checkIfInCodes(
                                          ...
                                          null)
                                        _copyCode(
                                            _rCoupons[i].id, _rCoupons[i].code);
                                    },
                                    child: Container(
                      ...
                      children: [
                        ...
                        Column(
                          children: [
                            InkWell(
                              onTap: () {
                                _handlePositiveButton(
                                    _rCoupons[i].id, _rRatings);
                              },
                              child: Container(
                                width: 50,
                                padding: const EdgeInsets.all(5),
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    //borderRadius: BorderRadius.only(bottomRight: Radius.circular(5),bottomLeft: Radius.circular(5)),
                                    borderRadius: BorderRadius.circular(5),
                                    color: homeApi.checkIfInRatings(
                                                _rCoupons[i].id,
                                                _rRatings,
                                                'pos') ==
                                            null
                        ...
                        InkWell(
                          onTap: () {
                            Share.share(getTranslated(context, 'share_text') +
                                cPropertyByLocale(
                                    context, _rCoupons[i], 'name') +
                                " " +
                                _rCoupons[i].code);
                          },
                          child: Container(
    ...
    homeApi.copyCode(cid).then((value) {
      if (value) {
        ...
        ClipboardManager.copyToClipBoard(code).then((result) {
          /*Fluttertoast.showToast(
              msg: getTranslated(context, getTranslated(context, 'Copied')),
              toastLength: Toast.LENGTH_SHORT,
              gravity: ToastGravity.CENTER,
              timeInSecForIosWeb: 1,
              backgroundColor: Colors.black,
  ...
  Future<void> _checkIfUserSelectCountry() async {
    final prefs = await SharedPreferences.getInstance();

    final key6 = 'country_code';
    final value6 = prefs.getString(key6);

    print ('country_code '+value6);
    if(value6.isEmpty || value6 == "" || value6 == null)
      {
        Navigator.pushReplacementNamed(context, '/SelectCountry');
  ...
  void initState() {
    ...
    _initializeStoresSection().whenComplete(() {

      homeApi.getLastCouponId().then((lastOne) {
        setState(() {
          _initializeCouponsSection(lastOne).whenComplete(() {
            homeApi.getUserFavorites().then((value) {
              setState(() {
                try {
                  _rFavorites = List.from(value);
                              ...
                              child: TextFormField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  contentPadding: EdgeInsets.only(
                                      top: 12), // add padding to adjust text
                                  isDense: true,
                                  hintText: getTranslated(
                                      context, 'home_search_hint'),
                                  hintStyle: TextStyle(
                                    color: Colors.black,
                                    fontSize: 16,
                                    //fontWeight: FontWeight.w600,
                ...
                Row(
                    ...
                    children: <Widget>[
                      Padding(
                          ...
                          child: Text(
                            getTranslated(context, 'home_stores_title'),
                            style: TextStyle(
                                fontSize: 16,
                                fontFamily: "CustomFont",
                                color: Colors.black),
                      ...
                      Padding(
                        ...
                        child: InkWell(
                            ...
                            child: Text(
                              getTranslated(context, 'home_stores_all_btn'),
                              style: TextStyle(
                    ...
                    children: <Widget>[
                      Padding(
                          padding: const EdgeInsets.only(
                              top: 15, right: 10, left: 10, bottom: 0),
                          child: Text(
                            getTranslated(context, 'home_coupons_title'),
                            style: TextStyle(
                                fontSize: 20,
                                fontFamily: "CustomFont",
                                color: Colors.black),
...
*/
                ...
                Visibility(
                  ...
                  child: Padding(
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text(
                          getTranslated(context, 'home_coupons_end_results'),
                          style: TextStyle(
                            fontFamily: 'CustomFont',
      ...
      bottomNavigationBar: StyleProvider(
          ...
          child: ConvexAppBar(
            ...
            items: [
              TabItem(
                  icon: MyIcons.globe,
                  title: getTranslated(context, 'home_b_bar_countries')),
              TabItem(
                  icon: Icons.shopping_bag,
                  title: getTranslated(context, 'home_b_bar_stores')),
              TabItem(
                  icon: Icons.add_box,
                  title: getTranslated(context, 'add_coupon_title')),
              TabItem(
                  icon: Icons.favorite,
                  title: getTranslated(context, 'home_b_bar_fav')),
              TabItem(
                  icon: Icons.home,
                  title: getTranslated(context, 'home_b_bar_home')),
            ],
  ...
  Future _getCountries() async {
    var csResponse = await http.get(
        'https://yalaphone.com/appdash/rest_api/countries/getAllCountries.php');
    var csData = json.decode(csResponse.body);
    Country tCountry;
    _countries = [];

    Map<String, dynamic> q = json.decode(
        '{"id":"0","ar-name":"كل الدول","en-name":"All countries","flag":""}');
  ...
  Future _updateCountries(String user_id, String country_id) async {
    ...
    String myUrl =
        "https://yalaphone.com/appdash/rest_api/countries/update_user_country.php";
